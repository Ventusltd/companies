#!/usr/bin/env python3
"""Deterministic boundary contract for the 202608271547 iXBRL ceiling repair."""
...
EXTRACT = load("companies_extract_202608271547", "build/python/202608271547-extract-bounded-accounts.py")
PLAN = load("companies_plan_202608271547", "build/python/202608271547-freeze-companies-house-plan.py")
DOWNLOAD = load("companies_download_202608271547", "build/python/202608271547-download-planned-archive.py")
VERIFY = load("companies_verify_202608271547", "build/python/202608271547-verify-companies-house-candidate.py")
...
def test_generation_boundary() -> None:
    expected_parent = "625101ef325f3d67fc866e3822bd76f1fcbb2e49"
    assert PLAN.GENERATION == DOWNLOAD.GENERATION == VERIFY.GENERATION == "202608271547"
    assert PLAN.BASE_COMMIT == DOWNLOAD.BASE_COMMIT == VERIFY.BASE_COMMIT == expected_parent
    assert PLAN.PREVIOUS.GENERATION == DOWNLOAD.PREVIOUS.GENERATION == VERIFY.PREVIOUS.GENERATION == "202608271547"
    assert PLAN.PREVIOUS.BASE_COMMIT == DOWNLOAD.PREVIOUS.BASE_COMMIT == VERIFY.PREVIOUS.BASE_COMMIT == expected_parent
    assert VERIFY.PREVIOUS.FIXED_GENERATED_AT == "2026-08-27T14:47:00Z"
...
def test_source_manifest() -> None:
    path = ROOT / "manifests/202608271547-bounded-companies-house-candidate.json"
    manifest = json.loads(path.read_text())
    assert manifest["generation"] == "202608271547"
    assert manifest["base_commit"] == "625101ef325f3d67fc866e3822bd76f1fcbb2e49"
