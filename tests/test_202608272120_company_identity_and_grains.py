#!/usr/bin/env python3
"""Source-only contracts for the 202608272120 Companies successor."""
...
PLAN = load("build/python/202608272120-freeze-companies-house-plan.py", "companies_plan_2120_test")
DOWNLOAD = load("build/python/202608272120-download-planned-archive.py", "companies_download_2120_test")
EXTRACT = load("build/python/202608272120-extract-bounded-accounts.py", "companies_extract_2120_test")
VERIFY = load("build/python/202608272120-verify-companies-house-candidate.py", "companies_verify_2120_test")
BASE_FIXTURE = load("tests/test_202608271507_bounded_companies_house.py", "companies_1507_fixture_for_2120")
...
def test_generation_and_fixed_plan() -> None:
    assert PLAN.GENERATION == DOWNLOAD.GENERATION == EXTRACT.GENERATION == VERIFY.GENERATION == "202608272120"
    assert PLAN.BASE_COMMIT == DOWNLOAD.BASE_COMMIT == VERIFY.BASE_COMMIT == "cd870ff53d2693b734e5860947bb0fa96bde9cf3"
    ...
    encoded_plan = (json.dumps(plan, indent=2, sort_keys=True) + "\n").encode()
    assert hashlib.sha256(encoded_plan).hexdigest() == VERIFY.EXPECTED_PLAN_SHA256

    with tempfile.TemporaryDirectory(prefix="companies-2120-plan-") as temporary:
        root = Path(temporary)
        plan_path = root / "plan.json"
        plan_path.write_bytes(encoded_plan)
        loaded_plan, loaded_item = DOWNLOAD.load_item(plan_path, 0)
        assert loaded_plan == plan
        assert loaded_item == PLAN.EXPECTED_FILES[0]
        rest_path = root / "rest-evidence.json"
        rest = PLAN.fixed_rest_evidence(rest_path)
        assert rest["generation"] == "202608272120"
        assert hashlib.sha256(rest_path.read_bytes()).hexdigest() == VERIFY.EXPECTED_REST_EVIDENCE_SHA256
...
def test_extractor_company_number_end_to_end() -> None:
    accepted = {
        "00000006": "Prod224_2605_00000006_20260331.html",
        "SC123456": "Prod101_2606_SC123456_20260430.xhtml",
        "R0000001": "Prod999_2607_R0000001_20260531.xml",
        "AB12CD34": "Prod000_2605_ab12cd34_20260331.HTML",
    }
    for number, member in accepted.items():
        assert EXTRACT.COMPANY_NUMBER.fullmatch(number)
        row, parser = EXTRACT.parse_document(member, ixbrl_payload(15_000_000))
        assert parser == "xml"
        assert row and row["company_number"] == number
        assert row["source_file"] == member
    for number in ("", "1234567", "123456789", "SC12345", "AB-12345"):
        assert not EXTRACT.COMPANY_NUMBER.fullmatch(number)
    # Every token around identity is closed. In particular, a valid company-like
    # second or fourth token can never be reinterpreted as the company key.
    for misleading_name in (
        # invalid producer token / prefix and ASCII lookalikes
        "Prod22_2605_00009872_20260331.html",
        "Prod2244_2605_00009872_20260331.html",
        "prefix_Prod224_2605_00009872_20260331.html",
        "prod224_2605_00009872_20260331.html",
        "Prød224_2605_00009872_20260331.html",
        # invalid second YYMM token
        "Prod224_265_00009872_20260331.html",
        "Prod224_26050_00009872_20260331.html",
        "Prod224_26O5_00009872_20260331.html",
        "Prod224_R0000001_00009872_20260331.html",
        # invalid third identity token
        "Prod224_2605_0009872_20260331.html",
        "Prod224_2605_000009872_20260331.html",
        "Prod224_2605_AB-2CD34_20260331.html",
        "Prod224_2605_ſC123456_20260331.html",
        "Prod224_2605_ıC123456_20260331.html",
        # invalid fourth date token, shifted identity bait, suffix or extension
        "Prod224_2605_00009872_2026033.html",
        "Prod224_2605_00009872_202603311.html",
        "Prod224_2605_00009872_2026033O.html",
        "Prod224_2605_00009872_R0000001.html",
        "Prod224_2605_00009872_20260331_extra.html",
        "Prod224_2605_00009872_20260331.htm",
        "unrelated_2605_00009872_20260331.html",
    ):
    ...
    # Directory placement is not identity: only the exact basename is parsed.
    row, parser = EXTRACT.parse_document(
        "nested/Prod224_2605_00009872_20260331.html", ixbrl_payload(15_000_000)
    )
    assert parser == "xml" and row and row["company_number"] == "00009872"

    with tempfile.TemporaryDirectory(prefix="companies-2120-extract-") as temporary:
        root = Path(temporary)
        archive = root / "Accounts_Monthly_Data-Fixture2026.zip"
        with zipfile.ZipFile(archive, "w", compression=zipfile.ZIP_STORED) as handle:
            handle.writestr("Prod224_2605_R0000001_20260331.xhtml", ixbrl_payload(15_000_001))
            handle.writestr("Prod224_2605_AB12CD34_20260331.xhtml", ixbrl_payload(15_000_002))
            handle.writestr("Prod224_R0000001_00009872_20260331.xhtml", ixbrl_payload(99_999_999))
...
def test_actual_landed_two_grain_readback() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="companies-2120-parquet-") as temporary:
        base = Path(temporary)
        first = base / "first"
        second = base / "second"
        first.mkdir()
...
def test_full_successor_seal_and_verify() -> None:
    with tempfile.TemporaryDirectory(prefix="companies-2120-seal-") as temporary:
        root = Path(temporary)
...
def test_source_manifest_and_workflow() -> None:
    source_path = ROOT / "manifests/202608272120-bounded-companies-house-candidate.json"
    source = json.loads(source_path.read_text())
    assert source["generation"] == "202608272120"
    assert source["parent_generation"] == "202608272035"
    ...
    assert source["company_number_domain"]["identity_token"] == 3
    assert source["company_number_domain"]["monthly_member_example"] == "Prod224_2605_00009872_20260331.html"
    assert source["company_number_domain"]["monthly_member_pattern"] == EXTRACT.COMPANY_NUMBER_IN_FILENAME.pattern
    ...
    workflow = (ROOT / ".github/workflows/202608272120-bounded-companies-house-candidate.yml").read_text()
    assert "workflow_dispatch" not in workflow
    assert "python tests/test_202608272035_company_identity_and_grains.py" in workflow
