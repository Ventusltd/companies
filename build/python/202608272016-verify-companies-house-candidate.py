#!/usr/bin/env python3
"""Seal and verify the 202608272016 JSON + Parquet candidate closure."""
...
FIXED_GENERATED_AT = "2026-08-27T19:16:00Z"
...
PREVIOUS.COMPANY_NUMBER = re.compile(r"^[A-Z0-9]{8}$")
...
def seal(*args, **kwargs) -> dict:
    ...
    manifest["source_licence"] = PREVIOUS.OGL
...
def verify(root: Path) -> dict:
    ...
    try:
        ...
        if manifest.get("usage_context") != "NON_COMMERCIAL_OPEN_SOURCE":
            errors.append("usage context")
        if manifest.get("source_licence") != PREVIOUS.OGL or manifest.get("source_rights_are_distinct_from_usage_context") is not True:
            errors.append("source rights separation")
