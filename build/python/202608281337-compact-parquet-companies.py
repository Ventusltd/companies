...
EXPECTED_MAIN_PARENT_COMMIT = "29fba0f2bc9053188b5634754f400faa012f2899"
SUPERSEDED_FAILED_RUN_ID = 33164961170
...
CONTRACT_PATH = Path("contracts/202608281337-compact-parquet-companies.json")
...
SOURCE_BOUNDARY = (
    ".github/workflows/202608281337-compact-parquet-companies-candidate.yml",
    "README.md",
    "build/python/202608281337-compact-parquet-companies.py",
    "contracts/202608281337-compact-parquet-companies.json",
    "tests/test_202608281337_compact_parquet_companies.py",
)
...
FORBIDDEN_EDGE_COLUMNS = frozenset(
    {
        "generation",
        "relationship_repository",
        "relationship_repository_commit",
        "company_name",
...
BASE = load_module(ROOT / BASE_PATH, "companies_compact_base_202608281337")
COMPILER = load_module(ROOT / COMPILER_PATH, "companies_compact_selector_202608281337")
ENGINE = BASE.ENGINE
LEGACY = BASE.LEGACY
    ...
    if (
        ...
        != {
            RELATIONSHIP_PARQUET.as_posix(): {
                ...
                "provenance_location": "manifest.inputs",
            },
            SOLAR_PARQUET.as_posix(): {
                ...
                "identity_posture": "CANDIDATE_ONLY_NOT_CONFIRMED_OWNERSHIP",
                "materialized_subset_of": RELATIONSHIP_PARQUET.as_posix(),
                "predicate": "pinned_repd.technology = 'solar'",
                "provenance_location": "manifest.inputs",
            },
        }
...
def select_relationship_records(
    ...
    if BASIC_SCAN_HEARTBEAT_ROWS < 1:
        raise RuntimeError("Basic-company scan heartbeat interval is invalid")
    next_heartbeat = BASIC_SCAN_HEARTBEAT_ROWS
    with zipfile.ZipFile(basic_archive) as archive:
        ...
        for member in csv_members:
            with archive.open(member) as stream:
                ...
                for header in rows.fieldnames or []:
                    if header is None:
                        raise RuntimeError("Basic-company CSV contains an invalid header")
                    normalized_headers[re.sub(r"[^a-z0-9]", "", header.lower())] = header
                number_header = normalized_headers.get("companynumber")
                name_header = normalized_headers.get("companyname")
                status_header = normalized_headers.get("companystatus")
                sic_headers = [
                    normalized_headers.get(f"siccodesictext{index}") for index in range(1, 5)
                ]
                for row in rows:
                    if None in row:
                        raise RuntimeError("Basic-company CSV row contains unexpected extra columns")
                    summary["basic_company_rows_scanned"] += 1
                    if summary["basic_company_rows_scanned"] == next_heartbeat:
                        print(
                            json.dumps(
                                {
                                    "event": "basic_company_scan_progress",
                                    "basic_company_rows_scanned": next_heartbeat,
                                    "selected_companies": summary["selected_companies"],
                                    "companies_with_repd_candidates": summary[
                                        "companies_with_repd_candidates"
                                    ],
                                    "candidate_relationship_rows": summary[
                        ...
                        next_heartbeat += BASIC_SCAN_HEARTBEAT_ROWS
                    number_value = row.get(number_header) if number_header is not None else ""
                    name_value = row.get(name_header) if name_header is not None else ""
                    status_value = row.get(status_header) if status_header is not None else ""
                    number = (number_value or "").upper()
                    name = name_value or ""
                    ...
                    if not (large or matches or probable_spv):
                        continue
                    sic_codes = [
                        (row.get(header) if header is not None else "") or ""
                        for header in sic_headers
                    ]
...
def edge_tuple(number: Any, ref: Any, evidence_type: Any) -> tuple[str, str, str]:
    if (
        ...
        raise RuntimeError("Relationship bridge identity drifted")
    return number, ref, evidence_type


def relationship_rows(
    records: list[dict[str, Any]], projects: dict[str, dict[str, Any]]
) -> list[tuple[str, str, str]]:
    result: list[tuple[str, str, str]] = []
    seen: set[tuple[str, str, str]] = set()
    for record in records:
        number = str(record.get("company_number", ""))
        edges = record.get("repd_name_candidates")
        if not isinstance(edges, list) or not edges:
            raise RuntimeError("Relationship bridge received a company without candidate edges")
        for edge in edges:
            if not isinstance(edge, dict):
                raise RuntimeError("Relationship bridge received a malformed candidate edge")
            ref = str(edge.get("repd_ref", ""))
            project = projects.get(ref)
            technology = project.get("technology") if isinstance(project, dict) else None
            if (
                not isinstance(project, dict)
                or technology not in REPD_TECHNOLOGIES
                or edge.get("technology") != technology
            ):
                raise RuntimeError(
                    "Relationship bridge foreign key or pinned REPD technology drifted"
                )
            row = edge_tuple(number, ref, str(edge.get("match_type", "")))
            if row in seen:
                raise RuntimeError(f"Duplicate relationship key: {'/'.join(row)}")
            seen.add(row)
            result.append(row)
    if not result:
...
def solar_rows(
    rows: list[tuple[str, str, str]], projects: dict[str, dict[str, Any]]
) -> list[tuple[str, str, str]]:
    result: list[tuple[str, str, str]] = []
    for row in rows:
        project = projects.get(row[1])
        if not isinstance(project, dict) or project.get("technology") not in REPD_TECHNOLOGIES:
            raise RuntimeError("Relationship bridge contains an unknown pinned REPD reference")
...
def audit_relational_integrity(
    ...
    try:
        ...
        dimension_rows = []
        for ref, project in sorted(projects.items()):
            if not isinstance(ref, str) or not ref or ref != ref.strip() or not isinstance(project, dict):
                raise RuntimeError("Pinned REPD dimension identity drifted")
            technology = project.get("technology")
            if technology is not None and not isinstance(technology, str):
                raise RuntimeError("Pinned REPD technology type drifted")
            dimension_rows.append((ref, technology))
        connection.executemany("INSERT INTO pinned_repd VALUES (?, ?)", dimension_rows)
        main = ENGINE.sql_path(relationship_path)
        solar = ENGINE.sql_path(solar_path)
        expected_solar = (
            f"SELECT m.* FROM read_parquet('{main}') m "
            "JOIN pinned_repd r USING (repd_ref) WHERE r.technology = 'solar'"
        )
        unknown_refs = int(
            connection.execute(
                f"SELECT count(*) FROM read_parquet('{main}') m "
                "LEFT JOIN pinned_repd r USING (repd_ref) WHERE r.repd_ref IS NULL"
            ).fetchone()[0]
        )
        outside_parent = int(
            connection.execute(
                f"SELECT count(*) FROM (SELECT * FROM read_parquet('{solar}') "
                f"EXCEPT SELECT * FROM read_parquet('{main}'))"
            ).fetchone()[0]
        )
        outside_predicate = int(
            connection.execute(
                f"SELECT count(*) FROM (SELECT * FROM read_parquet('{solar}') "
                f"EXCEPT {expected_solar})"
            ).fetchone()[0]
        )
        missing_expected = int(
            connection.execute(
                f"SELECT count(*) FROM ({expected_solar} EXCEPT "
                f"SELECT * FROM read_parquet('{solar}'))"
            ).fetchone()[0]
        )
    finally:
        connection.close()
    if unknown_refs or outside_parent or outside_predicate or missing_expected:
        raise RuntimeError("Solar bridge is not the exact pinned-REPD relational subset")
    return {
        "pinned_repd_join": True,
        "unknown_repd_refs": unknown_refs,
        "solar_rows_outside_parent": outside_parent,
        "solar_rows_outside_predicate": outside_predicate,
        "expected_solar_rows_missing": missing_expected,
...
def build_datasets(
    ...
    rows = relationship_rows(records, projects)
    selected_solar_rows = solar_rows(rows, projects)
    ...
    relational_integrity = audit_relational_integrity(
        output / RELATIONSHIP_PARQUET,
        output / SOLAR_PARQUET,
        projects,
    )
        ...
        "datasets": {
            ...
            "solar_company_repd_relationships": {
                **solar,
                ...
                "evidence_class": "CANDIDATE",
                "identity_posture": "CANDIDATE_ONLY_NOT_CONFIRMED_OWNERSHIP",
                "materialized_subset_of": "company_repd_candidates",
                "predicate": "pinned_repd.technology = 'solar'",
                "provenance_location": "manifest.inputs",
            },
        },
...
def read_relationship_rows(path: Path) -> list[tuple[str, str, str]]:
    duckdb = ENGINE.load_duckdb()
    connection = duckdb.connect(":memory:")
    result: list[tuple[str, str, str]] = []
    try:
        ...
        while True:
            ...
            for row in batch:
                ...
                expected = edge_tuple(*row)
                if tuple(row) != expected:
                    raise RuntimeError("Relationship landed typed bridge value drifted")
...
def exact_file_closure(root: Path, manifest: dict[str, Any]) -> tuple[list[Path], int]:
    ...
    if any(
        path.suffix.lower()
        in {".zip", ".ndjson", ".csv", ".gz", ".duckdb", ".db", ".sqlite", ".sqlite3"}
        for path in files
    ):
        raise RuntimeError("Raw, database or transport data leaked into the compact candidate")
...
def verify(root: Path, expected_source_commit: str, repd_root: Path) -> dict[str, Any]:
    ...
    projects, runtime_repd_manifest = LEGACY.repd_closure(repd_root)
    if runtime_repd_manifest != inputs.get("repd") or not BASE.verify_repd_input(
        runtime_repd_manifest
    ):
        raise RuntimeError("Pinned REPD runtime closure drifted during relationship readback")
    ...
    rows = read_relationship_rows(root / RELATIONSHIP_PARQUET)
    landed_solar_rows = read_relationship_rows(root / SOLAR_PARQUET)
    selected_solar_rows = solar_rows(rows, projects)
    if landed_solar_rows != selected_solar_rows:
        raise RuntimeError("Landed solar bridge differs from the exact pinned-REPD subset")
    relationships = ENGINE.audit_parquet(
    ...
    distinct_companies = len({row[0] for row in rows})
    if (
...
def build(
    ...
    audit = build_datasets(output, records, inputs["projects"])
    ...
    del records, inputs
    return verify(output, source_commit, repd_root)
...
def parse_args() -> argparse.Namespace:
    ...
    verify_parser.add_argument("--repd", type=Path, required=True)
